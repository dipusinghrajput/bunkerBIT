...
def save_credentials(username, password, required_percentage):
    ...
    with open(CSV_FILE, mode='a', newline='') as file:
        ...
        writer.writerow([username, password, required_percentage])

def login_and_fetch_attendance(username, password, required_percentage):
...
def login():
    ...
    save_credentials(username, password, required_percentage)

    # Now fetch attendance data (if login is successful)
    result = login_and_fetch_attendance(username, password, required_percentage)
    if "error" in result:
